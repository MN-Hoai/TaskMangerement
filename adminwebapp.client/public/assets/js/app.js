...
(function () {
	...
	function hideShowLayoutOptions(dataLayout) {
		if (dataLayout == "vertical") {
			...
			let themeSettingsOffcanvas = document.getElementById("theme-settings-offcanvas");
			...
			let themeSettingsOffcanvas = document.getElementById("theme-settings-offcanvas");
			if (themeSettingsOffcanvas) {
				let sidebarSize = document.getElementById("sidebar-size");
				if (sidebarSize) {
					sidebarSize.style.display = "none";
				}

				let sidebarView = document.getElementById("sidebar-view");
				if (sidebarView) {
					sidebarView.style.display = "none";
				}

				let sidebarColor = document.getElementById("sidebar-color");
				if (sidebarColor) {
					sidebarColor.style.display = "none";
				}

				let sidebarImg = document.getElementById("sidebar-img");
				if (sidebarImg) {
					sidebarImg.style.display = "none";
				}

				let layoutPosition = document.getElementById("layout-position");
				if (layoutPosition) {
					layoutPosition.style.display = "block";
				}

				let layoutWidth = document.getElementById("layout-width");
				if (layoutWidth) {
					layoutWidth.style.display = "block";
				}

				let sidebarVisibility = document.getElementById("sidebar-visibility");
				if (sidebarVisibility) {
					sidebarVisibility.style.display = "none";
				}
			}
	...
	function initAvatarUpload() {
		//Lưu ý: để chạy trong react fileupload cần loại bỏ thuộc tính value="" trong mã html 
		const avatarFileUpload = document.querySelector("#avatar-file-upload");
		let currentBlobUrl = null; // Lưu trữ Blob URL để quản lý và thu hồi
		...
		if (avatarFileUpload) {
			avatarFileUpload.addEventListener("change", function (event) {
				...
				if (file) {
					...
					// Gán Blob URL cho preview và input
					avatarPreview.src = currentBlobUrl;
					if (avatarUrl) {
						avatarUrl.value = `${currentBlobUrl}#${fileExtension}`;

						// Lưu extension vào thuộc tính data-type
					...
					if (avatarUrl) {
						...
						// Xóa thuộc tính data-type nếu không có file
						avatarUrl.removeAttribute("data-type");
					}
				}

				// Kích hoạt sự kiện input để React nhận biết thay đổi
				const eventInput = new Event("input", { bubbles: true });
				avatarUrl.dispatchEvent(eventInput);
	...
	// Global hàm initCKEditor để có thể gọi trong react
	...
	function initDropzone3() {
		...
		if (dropzonePreviewNode) {
			...
			const dropzone = new Dropzone(".dropzone", {
				...
				previewTemplate: previewTemplate,
				previewsContainer: "#dropzone-preview",
				autoProcessQueue: false, // Không tự động upload file
			});

			// Danh sách lưu trữ các file đã chọn
			let fileList = [];

			// Sự kiện khi file được thêm vào
			...
			// Sự kiện khi file bị xóa khỏi danh sách
			dropzone.on("removedfile", function (file) {
				console.log("File removed:", file);
				fileList = fileList.filter((f) => f !== file); // Loại bỏ file khỏi danh sách
				console.log("Current file list:", fileList);
			});

			// Ví dụ: Lấy danh sách file khi nhấn nút
			document.querySelector("#get-files-button").addEventListener("click", function () {
				console.log("Final file list:", fileList);
				alert(`You have selected ${fileList.length} file(s): ${fileList.map((f) => f.name).join(", ")}`);
			});
		}
	}
	...
	function initDropzone4() {
		...
		if (dropzonePreviewNode) {
			...
			// Lấy thẻ textarea để lưu danh sách Blob URL
			const galleryFilesUrl = document.querySelector(".gallery-files-url");
			const galleryFilesRemove = galleryFilesUrl.closest(".dropzon-container").querySelector(".gallery-files-remove");

			// Sự kiện khi file được thêm vào
			dropzone.on("addedfile", function (file) {
				console.log("File added:", file.name);

				// Tạo Blob URL từ file và thêm vào danh sách
				const fileUrl = URL.createObjectURL(file);
				fileList.push({ file, fileUrl }); // Lưu cả file và Blob URL

				// Cập nhật nội dung của textarea
				updateGalleryFileUrl();
			});
			...
			dropzone.on("removedfile", function (file) {
				...
				// Tìm và loại bỏ file khỏi danh sách
				const index = fileList.findIndex((item) => item.file === file);
				if (index !== -1) {
					const removedFile = fileList.splice(index, 1)[0]; // Loại bỏ file khỏi danh sách
					URL.revokeObjectURL(removedFile.fileUrl); // Thu hồi Blob URL để giải phóng bộ nhớ
				}
			...
			// Handle remove button click (xóa tất cả danh sách file đang chờ)
			if (galleryFilesRemove) {
				galleryFilesRemove.addEventListener("click", function (event) {
					event.preventDefault();

					// Lặp qua danh sách các files preview trong dropzone, tiến hành: 1. Thu hồi Blob URL, 2. Xóa files preview khỏi danh sách dropzone, 3. cập nhật nội dung của input chứa danh sách url

					// Kích hoạt sự kiện input của galleryFilesUrl để React nhận biết thay đổi
					const eventInput = new Event("input", { bubbles: true });
					galleryFilesUrl.dispatchEvent(eventInput);
				});
			}

			// Hàm cập nhật nội dung của input chứa danh sách url
			function updateGalleryFileUrl() {
				console.log(fileList);

				// Tạo danh sách Blob URL từ fileList
				const blobUrls = fileList.map((item) => item.fileUrl).join("#");

				// Cập nhật nội dung của textarea
				galleryFilesUrl.value = blobUrls;
			}
		}
	}
		...
		if (dropzonePreviewNode) {
			...
			if (galleryFilesRemove) {
				galleryFilesRemove.addEventListener("click", function (event) {
					...
					// Xóa tất cả file
					dropzone.removeAllFiles(true); // Xóa tất cả file trong Dropzone

					// Thu hồi toàn bộ Blob URL và xóa danh sách file
					fileList.forEach((item) => {
						URL.revokeObjectURL(item.fileUrl); // Thu hồi Blob URL
					});

					fileList = []; // Xóa toàn bộ file khỏi danh sách
	...
	function initInputMask() {
		...
		}).mask(document.querySelectorAll('.date-time-mask'));
    }
	...
	// Global hàm init để có thể gọi trong react
	window.init = init;
